function [binSurf,binSurfROIS, binSurfROIM, binSurfROIL] = findSurftib(im, ROIthresh)
...
% % Tibial ------------ ROIS = small ROI, ROIM = medium ROI, ROIL = large ROI
% 
ROIS = coordsSurf(1,:)>minL+ROIthresh(1)*l & coordsSurf(1,:)<minL+(1-ROIthresh(1))*l & ...
    coordsSurf(2,:)>minW+ROIthresh(1)*w & coordsSurf(2,:)<minW+(1-ROIthresh(1))*w;
ROIM = coordsSurf(1,:)>minL+ROIthresh(2)*l & coordsSurf(1,:)<minL+(1-ROIthresh(2))*l & ...
    coordsSurf(2,:)>minW+ROIthresh(2)*w & coordsSurf(2,:)<minW+(1-ROIthresh(2))*w;
ROIL = coordsSurf(1,:)>minL+ROIthresh(3)*l & coordsSurf(1,:)<minL+(1-ROIthresh(3))*l & ...
    coordsSurf(2,:)>minW+ROIthresh(3)*w & coordsSurf(2,:)<minW+(1-ROIthresh(3))*w;
